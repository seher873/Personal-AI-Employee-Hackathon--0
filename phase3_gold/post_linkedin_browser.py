#!/usr/bin/env python3
"""
LinkedIn Browser Poster - Gold Tier
====================================
Post to LinkedIn using Browser Automation (Playwright)
NO API permissions needed!
Uses credentials.json for login

Usage:
    py post_linkedin_browser.py "Your post text here"
    py post_linkedin_browser.py "Text" --image "path/to/image.jpg"
"""
...
def load_credentials():
    ...
    if not cred_file.exists():
        ...
        print('  {"email": "your@email.com", "password": "your_password", "profile_url": "your_profile"}')
        sys.exit(1)
...
def post_to_linkedin(text: str, image_path: str = None):
    ...
    print(f"[OK] Credentials loaded for: {email}")
    ...
    try:
        with sync_playwright() as p:
            ...
            if storage_state.exists():
                ...
                print("[INFO] No saved session, will login...")
                context = browser.new_context(
                    viewport={'width': 1920, 'height': 1080},
            ...
            if page.is_visible('button:has-text("Start a post")', timeout=5000):
                ...
                print("[INFO] If login fails, manually login in the browser window")
                print("[INFO] Session will be saved for next time")
                ...
                # Wait longer for login and potential 2FA/CAPTCHA
                print("[INFO] Waiting for login... (up to 60 seconds)")
                print("[INFO] Complete any CAPTCHA or 2FA if shown")

                # Wait for feed to load (longer timeout for manual intervention)
                try:
                    ...
                    print("[INFO] Script will wait up to 2 minutes")
            ...
            if image_path and os.path.exists(image_path):
                print(f"[5/5] Uploading image: {image_path}...")
                try:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print('  py post_linkedin_browser.py "Your post text"')
        print('  py post_linkedin_browser.py "Text" --image "image.jpg"')
        sys.exit(1)
